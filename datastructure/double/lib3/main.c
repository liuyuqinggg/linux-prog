...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        res = handler->llist_insert(handler,&tmp,LLIST_BACKWARD);
        if(res)
            exit(1);
    }
    
    
    handler->llist_travel(handler,print_s);
    ...
    std = handler->llist_find(handler,name,name_cmp); //为什么去掉handler->就不能调用。。、
    if(std != NULL)
        print_s(std);
    else    
        printf("can not find\n");
    ...
    res = handler->llist_delete(handler,name,name_cmp);
    ...
    res = handler->llist_fetch(handler,&id,id_cmp,std);
     if(res == 0)
    {
        handler->llist_travel(handler,print_s);
         printf("--------------------------\n");
