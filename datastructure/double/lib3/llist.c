...
LLIST *llist_create(int initsize)
{
    ...
    new->llist_insert = llist_insert;
    new->llist_find = llist_find;
    new->llist_delete = llist_delete;
    new->llist_fetch = llist_fetch;
    new->llist_travel = llist_travel;
...
int llist_insert(LLIST *l, const void *data, int mode)
{
    struct llist_node_st *new = malloc(sizeof(struct llist_node_st) + l->size);//重点
    if(new == NULL)
        return -1;
