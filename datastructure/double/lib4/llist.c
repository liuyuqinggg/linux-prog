...
{
    struct llist_head_st *l = list;
    struct llist_node_st *new = malloc(sizeof(struct llist_node_st) + l->size);
    if(new == NULL)
        return -1;
...
static struct llist_node_st *find_(struct llist_head_st *l, const void *key,llist_cmp *cmp)
{
...
void llist_travel(LLIST *list, llist_op *print_s)
{
    struct llist_head_st *l = list;
